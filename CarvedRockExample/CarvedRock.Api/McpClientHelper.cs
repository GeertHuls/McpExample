...
public static class McpClientHelper
{
        ...
        IConfiguration config, CancellationToken cxl)
    {
        ...
        var accesToken = await GetAccessTokenFromHttpContext(httpCtxAccessor, config);
        clientTransport.AdditionalHeaders.Add("Authorization", accesToken);
    ...
    {
        return config.GetValue<string>("services__mcp__http__0") // service discovery setup from Aspires
            ?? config.GetValue<string>("MCP_HTTP")
    ...
    private static async Task<string> GetAccessTokenFromHttpContext(IHttpContextAccessor httpCtxAccessor, IConfiguration config)
    {
                    ...
                    StringComparison.OrdinalIgnoreCase))
        {
            ...
            var newAccessToken = await GetDelegatedAccessTokenAsync(accessToken, config);
    ...
    private static async Task<string> GetDelegatedAccessTokenAsync(string accessToken, IConfiguration config)
    {
